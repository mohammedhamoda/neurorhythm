...
const GuitarString = ({ note, isActive, onClick, index }) => {
  ...
  return (
    <button
      ...
      className={`
        relative flex-1 h-64 md:h-80 
        flex flex-col justify-center items-center 
        transition-all duration-100 ease-out touch-manipulation group
        border-r border-amber-900/30 first:border-l
    ...
    >
      <div 
        ...
        ${isActive 
          ? 'bg-cyan-200 shadow-[0_0_15px_rgba(34,211,238,0.8)] scale-x-125' 
          : 'bg-stone-400 shadow-black/20'
        }`}
        style={{ 
          width: `${stringThickness}px`,
          transform: isActive ? 'translateX(0px) scale(1.2)' : 'translateX(0px)'
        }} 
      />
      ...
      <div className={`
        ...
        ${isActive 
          ...
          : 'border-stone-500/10 text-stone-500/0 opacity-0 group-hover:opacity-30'
        }
      `}>
        {note}
      </div>
